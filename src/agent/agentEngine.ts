...
/** 单轮任务联网搜索调用次数上限（V1.5.0 引入；V1.5.5 多轮自主纠错仍受总轮次严格控制，避免无意义反复搜索） */
...
7. 联网搜索（web_search，可用时）：仅在需要时效性信息（最新技术文档、版本特性、实时报错解决方案）或外部知识（第三方库 API 用法、行业标准、本地未覆盖的技术方案）时调用；常规本地代码修改、已有文件问答、纯逻辑推理等无需外部信息的场景禁止搜索
   - 查询词策略：先提炼任务核心诉求再转为精准关键词，禁止宽泛笼统；技术类问题必须附带技术栈、版本号、报错信息等限定词缩小召回范围；可附带 altQueries 提供最多 2 组差异化查询词互补召回
   - 结果条数：按任务复杂度通过 maxResults 自主决定（2-20），简单明确查询取 2-5 条，复杂多维度查询可扩展；单轮信息不足优先二次搜索补充而非单轮无限扩容
   - 自主纠错：首次结果质量不足时分析信息缺口，调整关键词后发起补充检索；信息足够或确认无公开信息即停止，同一关键词不重复搜索
   - 搜索结果已精简压缩且链接已经过有效性校验，引用时注明来源；搜索失败不影响任务，回退基于已有知识作答
